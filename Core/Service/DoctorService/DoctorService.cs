...
namespace Service.DoctorService
{
	public class DoctorService(IUnitOfWork unitOfWork, IMapper mapper, UserManager<User> userManager) : IDoctorService
	{
		...
		public async Task<IEnumerable<MedicalRecordDto>> GetAllMedicalRecordsAsync(int doctorId)
		{
			var medicalrecords = await unitOfWork.GetRepository<MedicalRecord, Guid>().GetAllAsync(new MedicalRecordWithRadiologyAndLapTest(doctorId));
			return mapper.Map<IEnumerable<MedicalRecordDto>>(medicalrecords);
		}
		...
		public async Task<MedicalRecordDto> UpdateMedcalRecordAsync(UpdateMedicalRecordDto medicalRecord)
		{
			var record = await unitOfWork.GetRepository<MedicalRecord, Guid>().GetAsync(medicalRecord.Id);
			if (record == null) throw new MedicalRecordNotFoundException(medicalRecord.Id);

			record.Prescription = medicalRecord.Prescription;
			record.Diagnoses = medicalRecord.Diagnoses;

			unitOfWork.GetRepository<MedicalRecord,Guid>().Update(record);
